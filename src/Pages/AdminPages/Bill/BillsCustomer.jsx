...
export default function BillsCustomer({ match }) {
    ...
    useEffect(() => {
        axios.get(`${URL}/data/bill/iduser=${match.match.params.idCustomer}`)
            .then(res => {
                if (bill === null) {
                    setBills(res.data);
                } else {
    ...
    useEffect(() => {
        axios.get(`${URL}/data/user/${match.match.params.idCustomer}`, null)
    ...
    const acceptBill = () => {
        ...
        axios.get(`${URL}/data/bill/action=accept/${bill.id}`, null)
            .then((res) => {
                reLoad();
                alert("Đã xác nhận");
            })
    ...
    const deleteBill = () => {
        if (window.confirm("Đơn hàng chưa được xác nhận ! thao tác này sẽ xóa đơn hàng ! Xác nhận xóa ?\n")) {
            axios.get(`${URL}/data/bill/action=cancel/${bill.id}`, null)
                .then((res) => {
                    setBill(null);
                    reLoad();
    ...
    const deleteBillDetail = (idBill, idProduct) => {
        if (bill.billDetails.length === 1) {
            ...
            if (window.confirm("Xác nhận xóa 1 sản phẩm trong đơn hàng")) {
                axios.get(`${URL}/data/bill/action=delete/billdetail/idbill=${idBill}/idproduct=${idProduct}`)
                    .then(res => {
                        if (res.status === 204) {
                            reLoad();
                        }
